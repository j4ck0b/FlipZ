...
export default function ListingModal({ open, onClose, onSuccess, editListing = null, defaultCategory = null }) {
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      for (const file of filesToUpload) {
        // Upload original
        const uploadResult = await flipzApi.integrations.Core.UploadFile({ file });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editListing) {
        await flipzApi.entities.CardListing.update(editListing.id, data);
        toast.success(t('listingUpdated'));
      } else {
        await flipzApi.entities.CardListing.create(data);
        toast.success(
